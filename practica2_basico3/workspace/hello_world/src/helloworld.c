...
/*   Objetivo:
	...
	C5: CÁLCULO POTENCIA CLIMATIZACIÓN (FPGA)
	...
	Restricciones:
		...
		R3a: CAMBIO PROGRESIVO DE PERSIANAS Y VENTANAS
				También se controlará en el interior del MicroBlaze la acción sobre los motores de las persianas y ventanas
		...
		R3b: VISUALIZACIÓN CAMBIO PROGRESIVO DE PERSIANAS Y VENTANAS
		...
		R4a: LLAMADA DE MICROBLAZE A FPGA
			Se controlará en la parte de la FPGA el cálculo de la potencia correspondiente a los motores
		...
		R5. El alumno será responsable de crear las funciones lineales de control de la potencia
			de la calefacción y del aire acondicionado para que cumplan con los requisitos establecidos.
		...
		R6: AUTOINCREMENTOS TEMPERATURA
			Igualmente, el alumno deberá implementar en el MicroBlaze un sistema lógico de “autoincrementos” o “autodecrementos”
...
/*	Mejoras:
		...
		M4: Visualización del sensor/actuador activo en el display
		M5: Visualización de Número de ejecuciones de control de actuadores, llamadas a la fpga y refrescos de pantalla
		M6: Animación de movimiento de persianas y ventanas
		M7: Refresco máximo de pantalla cada décima de segundo
		*/
...
//==================================== VARIABLES GLOBALES
	...
	//- R2: ALMACENAMIENTO DE DATOS
		...
		st_Temporizador Tem;					// temporizadores y semáforos



//==================================== GESTIÓN DE REACCIÓN DE ACTUADORES ANTE CAMBIOS EN SENSORES

	//- Act_Calc_Pot_Clima: Calcular la potencia de climatización que sería necesaria con la temperatura interior actual
	...
	//- Act_Manejador_Pot_Clim: Manejador respuesta asíncrona del cálculo de potencia
...
//==================================== GESTIÓN DE INTERFAZ DE USUARIO - TECLADO Y BOTONES
		#ifdef MEJORAS
	...
	//- UI_SimularSensores: modifica el valor de los sensores a petición del usuario
		void UI_SimularSensores() {
			...
			//- Tratamiento de los botones/teclas de dirección
	            ...
	            if ((ui.sig_tecla==TECLA_IZQ) || (ui.sig_tecla==TECLA_DER) || (ui.sig_tecla==TECLA_ARR) || (ui.sig_tecla==TECLA_ABA)) {
	            	...
	            	//- Cambiar circularmente el número de sensor/actuador activo con las teclas de izquierda y derecha
					...
					//- el modo sensores se activa cuando el activo está por debajo del umbral entre sensores y actuadors (valor 5)
...
//==================================== GESTIÓN DE SENSORES
	//- Sen_ActualizarEstados(): modifica el estado de los sensores en función de los valores y los umbrales
...
//==================================== GESTIÓN DE INTERFAZ DE USUARIO - PANTALLA Y DISPLAY
	...
	//- UI_RefrecarPantalla: Actualiza la pantalla sólo si el semáforo indica que hubo algún cambio
			...
			//- U2a: CONTENIDO PANTALLA
				...
				//- Visualizar todos los sensores y actuadores
		            ...
		            for (iSenAct = 0; iSenAct < NUM_SEN_ACT ; iSenAct++) {
		            	marca_activo = (ui.sa_activo==iSenAct) ? '*' : ' '; // si está activo marca con *
...
//==================================== GESTIÓN DEL TEMPORIZADOR
	...
	//- Tem_Manejador_FIT1: interrupciones del temporizador FIT1, programado a 0.1 segundos
		...
		void Tem_Manejador_FIT1 (void* ref)	{
        	...
        	//- Incrementar el número de tics
        		Tem.tics_pres++; // R1: aumentar tiempo desde el último cambio del sensor de presencia
        		Tem.tics_auto++; // R6: aumentar tiempo desde el último autoincremento de temperatura
        		#ifdef MEJORAS	 // M7: Refresco máximo de pantalla cada décima de segundo
	        	Tem.tics_pant++;
	        	#endif
	...
	//- Tem_ActualizarTics: ajusta sensores y actuadores periódicamente
...
//==================================== INICIALIZACIÓN
	...
	void Inicializar() {
		...
		//- Inicializar actuadores
			Act_pot_clima = 0; 				// potencia de climatización respuesta de la fpga a cero

		//- Inicializar la interfaz de usuario
			ui.ult_tecla = ' '; 			// última tecla pulsada a visualizar en pantalla
			ui.sig_tecla = '\0'; 			// no se ha pulsado otra tecla desde la última vez
